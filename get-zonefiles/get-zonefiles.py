...
def update_zone(zone, name, zoneinfo):
    ...
    if old_zoneinfo:
        ...
        # in the next run.
        elif datetime.datetime.now(old_serial_uat.tzinfo) < old_serial_uat + datetime.timedelta(
            minutes=1
        ):
            logger.info(f"{name}: less than a minute since last serial {old_serial_uat}, skipping")
            return False
    return True
